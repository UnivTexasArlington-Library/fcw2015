...
$year = 2014;
include("../db.php");
//CTRLF all education and replace with college name
...
?>
	...
	<div id="rightnav">
	<ul id="nav">
	<div id="topTitle"><?php echo $college_name; ?></div><li class="current education_nav"><a href="#topcollege" id="top"></a></li>
	</ul>
	<ul id="subnav">
	<div id="curriculumTitle">Curriculum and Instruction</div><li class="education_nav"><a href="#7" id="curriculum"></a></li>
	<div id="leadershipTitle">Educational Leadership and Policy Studies</div><li class="education_nav"><a href="#8" id="leadership"></a></li>
	...
	if ($rowcount > 0) {
		echo "<div id='deanTitle'>Office of the Dean</div><li class='education_nav'><a href='#36' id='dean'></a></li>";
	}
	?>
	</ul>
	...
	<section id="topcollege">
		<div class="row">
			<h1 class='education collegehome'><?php echo $college_name; ?></h1>
	    </div>
	</section>
	...
	while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
		if ($row['department_id'] != $deptid) {
				echo '<section id="' . $row['department_id'] . '" class="education_departments" data-speed="4" data-type="background">';
				echo "<div class='row main-education'><div class='col-xs-11'>";
	...
	/*$stmt4 = $db->prepare("SELECT * FROM faculty, departments, items WHERE departments.department_id=faculty.department_id AND faculty.department_id=36 AND faculty.college_id=:id and faculty.faculty_id=faculty_items.faculty_id and faculty_items.faculty_id IN (Select distinct(faculty_id) FROM faculty_items)");*/
	/*$stmt4 = $db->prepare("SELECT * FROM faculty, departments, items, faculty_items WHERE departments.department_id=faculty.department_id AND faculty.department_id=36 AND faculty.college_id=:id AND faculty.faculty_id IN (SELECT DISTINCT(faculty_items.faculty_id) FROM `faculty_items` , items WHERE faculty_items.item_id=items.item_id AND items.item_date=2014)");*/
	...
	$rowcount = $stmt4->rowCount();
	if ($rowcount > 0) {
			echo '<section id="36" class="education_departments" data-speed="4" data-type="background">';
				echo "<div class='row main-education'><div class='col-xs-11'>";
				echo "<div class='depthead'>";
				echo "<div class='depttop'>";
			echo "<h2 class='smallcol visible-xs visible-sm'>" . $college_name . "</h2>";
   ...
   <script src="../deploy/scripts/jquery.min.js"></script>
   		...
   		$(document).ready(function() {   			
   			$("#subnav").onePageNav({
   				...
   				changeHash: true,
   					...
   					$(".education_departments").removeClass('curr');
	...
	$("#arrow").on('click', function(e) {
	   		e.preventDefault();
	   		var thishref = $('.education_nav.current').children('a').attr('href');
	   			if (thishref) {
   	...
   	$("#curriculum").hover( function() {
   		$("#curriculumTitle").toggle('slide');
   	});
   	$("#leadership").hover( function() {
   		$("#leadershipTitle").toggle('slide');
   	});
   	$("#top").hover( function() {
   	...
   	<script>
   		$(document).ready(function(){
	   		...
	   		$(".education_departments").css("min-height", inner);
	   		$(".education_departments").each(function() {
