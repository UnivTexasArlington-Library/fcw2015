...
$year = 2014;
include("../db.php");
//CTRLF all engineering and replace with college name
...
?>
	...
	<div id="rightnav">
	<ul id="nav">
	<div id="topTitle"><?php echo $college_name; ?></div><li class="current engineering_nav"><a href="#topcollege" id="top"></a></li>
	</ul>
	<ul id="subnav">
	...
	while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
		if ($row['department_id'] != $deptid) {
				echo '<section id="' . $row['department_id'] . '" class="engineering_departments" data-speed="4" data-type="background">';
				echo "<div class='row main-engineering'><div class='col-xs-11'>";
	...
	$rowcount = $stmt4->rowCount();
	if ($rowcount > 0) {
			echo '<section id="36" class="engineering_departments" data-speed="4" data-type="background">';
				echo "<div class='row main-engineering'><div class='col-xs-11'>";
				echo "<div class='depthead'>";
				echo "<div class='depttop'>";
			echo "<h2 class='smallcol visible-xs visible-sm'>" . $college_name . "</h2>";
   ...
   <script src="../deploy/scripts/jquery.min.js"></script>
	...
	$("#arrow").on('click', function(e) {
   	...
   	<script>
   	$("#bioengineering").hover( function() {
   		$("#bioengineeringTitle").toggle('slide');
   	});
   	$("#civil").hover( function() {
   		$("#civilTitle").toggle('slide');
   	});
   	$("#compsci").hover( function() {
   		$("#compsciTitle").toggle('slide');
   	});
   	$("#electrical").hover( function() {
   		$("#electricalTitle").toggle('slide');
   	});
   	$("#industrial").hover( function() {
   		$("#industrialTitle").toggle('slide');
   	});
   	$("#materials").hover( function() {
   		$("#materialsTitle").toggle('slide');
   	});
   	$("#mechanical").hover( function() {
   		$("#mechanicalTitle").toggle('slide');
   	});
   	$("#top").hover( function() {
   	...
   	<script>
   		$(document).ready(function(){
   			...
   			var winwidth = $(window).width();
	   		$(".engineering_departments").css("min-height", inner);
	   		$(".engineering_departments").each(function() {
