...
$year = 2014;
include("../db.php");
//CTRLF all urban and replace with college name
...
?>
	...
	<div id="rightnav">
	<ul id="nav">
	<div id="topTitle"><?php echo $college_name; ?></div><li class="current urban_nav"><a href="#topcollege" id="top"></a></li>
	<div id="facultyTitle">Urban and Public Affairs</div><li class="urban_nav"><a href="#urban_departments"  id="faculty"></a></li>
	</ul>
	...
	<section id="topcollege">
		<div class="row">
			<h1 class='urban collegehome'><?php echo $college_name; ?></h1>
	    </div>
	</section>
	...
	<?php 
				...
				echo '<section id="urban_departments" data-speed="4" data-type="background">';
				echo "<div class='row main-urban'><div class='col-xs-11'>";
				echo "<div class='depthead'>";
				echo "<div class='depttop'>";
				echo "<h2 class='hidden-xs hidden-sm'>Urban and Public Affairs</h2>";
			...
			$stmt = $db->prepare("SELECT faculty.faculty_id, faculty_img, faculty_first_name, faculty_last_name, faculty_middle_name, faculty_item_types, faculty_link FROM faculty, items, faculty_items, departments, colleges WHERE faculty.faculty_id=faculty_items.faculty_id AND items.item_id=faculty_items.item_id AND faculty.college_id=colleges.college_id AND colleges.college_id=:id AND item_date = :year ORDER BY faculty_last_name, faculty.faculty_id");
	    		...
	    		<div class='modal-content'><div class='modal-header'><button type='button' class='close' data-dismiss='modal' aria-hidden='true'>x</button><a href='" . $row['faculty_link'] . "' target='_blank'><h3>" . $row['faculty_first_name'] . " " . $row['faculty_middle_name'] . " " . $row['faculty_last_name'] . "</h3></a></div>";
  ...
  <script>
   	...
   	<script>
   		$(document).ready(function(){
	   		...
	   		$("#urban_departments").css("min-height", inner);
	   		$("#urban_departments").each(function() {
...
$(window).bind("resize scrollStop", function() {
   	...
   	$("#faculty").hover( function() {
		...
		$("#arrow").on('click', function(e) {
	   		e.preventDefault();
	   		var thishref = $('.urban_nav.current').children('a').attr('href');
	   			if (thishref) {
